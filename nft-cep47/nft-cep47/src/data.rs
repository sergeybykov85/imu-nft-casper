...
pub fn emit(event: &CEP47Event) {
    ...
    match event {
        ...
        CEP47Event::MinterUpdate { recipient } => {
            let mut param = BTreeMap::new();
            param.insert(CONTRACT_PACKAGE_HASH, package.to_string());
            param.insert("event_type", "cep47_minter_update".to_string());
            param.insert("recipient", recipient.to_string());
            events.push(param);
        }
        CEP47Event::AdminUpdate { recipient } => {
            let mut param = BTreeMap::new();
            param.insert(CONTRACT_PACKAGE_HASH, package.to_string());
            param.insert("event_type", "cep47_admin_update".to_string());
            param.insert("recipient", recipient.to_string());
            events.push(param);
        }                 
    };
